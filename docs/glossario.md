...
| **Tema oficial** | Um dos layouts mantidos pela plataforma (5 no MVP) |
| **Tema custom** | Layout próprio do tenant (upload + aprovação) — pós-MVP |
...
| **Fila WhatsApp** | Jobs de envio enfileirados no Redis e consumidos por worker |
| **Lock de agenda** | Trava de curta duração (Redis) no corretor/slot para evitar double-booking |
