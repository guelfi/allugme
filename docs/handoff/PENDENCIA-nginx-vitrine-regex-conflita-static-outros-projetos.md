...
2. ⏳ **Pendente (infra).** Rodar `deploy/apply-nginx-allugme.py` no host da OCI (ou aguardar o próximo deploy do Allugme, que já executa esse script via `scripts/deploy-oci.sh` / `.github/workflows/deploy-oci.yml`) e validar com `nginx -t` + reload.
...
4. ⏳ **Pendente (opcional).** Depois de corrigido e implantado (item 2), remover a location `^~ /static/` paliativa do Batuara é opcional — ela é inofensiva e serve como defesa extra, mas não é mais estritamente necessária se a regex for corrigida.
